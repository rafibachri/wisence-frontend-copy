...
import { deleteDataDailyRole } from "../../actions/data";
import { GrFormNext } from "react-icons/gr";
import { GrFormPrevious } from "react-icons/gr";
import { MdOutlineDeleteOutline } from "react-icons/md";
import Swal from "sweetalert2";

const ListWrapperRole = (props) => {
    const navigate = useNavigate();
    const location = useLocation();

    const { img, title, path, url, exportFilename, allowAdd, allowDelete, allowExport, allowFilter, filterSearch, filterDate, allowSearch, bulkAction, handleBulkAction, columns, data, refreshData, exportData, deleteDataRole, role, roles, condition, allowEdit, handleCustom, handleAdditionalFilter } = props;
    ...
    const handleCheckedAll = () => {
        ...
        list.forEach((item) => {
            checkedItem[item.roleID] = checkedAll ? false : true;
    ...
    const handleBulk = (e, type) => {
        e.preventDefault();
        const selectedItems = Object.keys(checkedItem).filter((roleID) => checkedItem[roleID]);
        ...
        Swal.fire({
            ...
            if (succeed.value) {
                for (let roleID of Object.keys(checkedItem)) {
                    if (checkedItem[roleID]) {
                        if (type === "hapus")
                            await deleteDataRole({ url, roleID }).then(() => {
                                refreshData({ url, search, page, limit, sort, filterSearch, rangeDate });
                            });
                        else await handleBulkAction(e, type, roleID);
                    }
                }
                setCheckedAll(false);
                setCheckedItem([]);
    ...
    const handleView = (e, roleID) => {
        e.preventDefault();
        navigate(`${path}/${roleID}/edit?return_url=${encodeURIComponent(location.pathname)}`);
    };
                        ...
                        {(allowDelete === undefined || allowDelete || bulkAction !== undefined) && (
                            <td className="text-center align-middle">
                                <input
                                    type="checkbox"
                                    name={item.roleID}
                                    checked={checkedItem[item.roleID] !== undefined && checkedItem[item.roleID]}
                                    onChange={handleClick}
                                />
                            </td>
                        )}
                        <td className="text-center align-middle data-item" onClick={(e) => handleView(e, item.roleID)}>
                            {page * limit + index + 1}
                        </td>
                        {columns.map((col, key) => {
                            ...
                            return (
                                <td
                                    ...
                                    onClick={(e) => (col.link === undefined ? handleView(e, item.roleID) : null)}
                                >
                            ...
                            <div className="card-text" style={{ padding: 5, width: '100%' }}>
                                <div className="d-flex" style={{ padding: 10 }} >
                                    ...
                                    <div>
                                        {columns.map((col, key) => {
                                            if (key === 0)
                                                return (<div key={key} className="module-card-title" onClick={(e) => handleView(e, item.roleID)}>{renderValue(col, item[col.key], item)}</div>)
...
export default connect(mapStateToProps)(ListWrapperRole);
