...
import { FaCheck, FaFilter, FaPlus, FaPrint, FaSearch } from "react-icons/fa";
...
import { deleteDataWifi } from "../../actions/data";
import { GrFormNext } from "react-icons/gr";
import { GrFormPrevious } from "react-icons/gr";
import { MdOutlineDeleteOutline } from "react-icons/md";
import Swal from "sweetalert2";

const ListWrapperWifi = (props) => {
    const navigate = useNavigate();
    const location = useLocation();

    const { img, title, path, url, exportFilename, allowAdd, allowCheck, allowDelete, allowExport, allowFilter, filterSearch, filterDate, allowSearch, bulkAction, handleBulkAction, columns, data, refreshData, exportData, deleteDataWifi, role, roles, condition, allowEdit, handleCustom, handleAdditionalFilter, handleCheckContract } = props;
    ...
    const handleCheckedAll = () => {
        ...
        list.forEach((item) => {
            checkedItem[item.locationID] = checkedAll ? false : true;
            // checkedItem[item.locationID] = checkedAll ? false : true;
        });
    };
    ...
    const handleBulk = (e, type) => {
        e.preventDefault();
        const selectedItems = Object.keys(checkedItem).filter((locationID) => checkedItem[locationID]);
        ...
        Swal.fire({
            ...
            if (succeed.value) {
                for (let locationID of Object.keys(checkedItem)) {
                    if (checkedItem[locationID]) {
                        if (type === "hapus")
                            await deleteDataWifi({ url, locationID }).then(() => {
                                refreshData({ url, search, page, limit, sort, filterSearch, rangeDate });
                            });
                        else await handleBulkAction(e, type, locationID);
                    }
                }
                setCheckedAll(false);
                setCheckedItem([]);
    ...
    const handleView = (e, locationID) => {
        e.preventDefault();
        navigate(`${path}/${locationID}/edit?return_url=${encodeURIComponent(location.pathname)}`);
    };
        ...
        return (
            <div className="d-flex left-flex">
                {(allowAdd === undefined || allowAdd) && isCreate && (
                    <Link to={`${path}/create?return_url=${encodeURIComponent(location.pathname)}`} className="btn btn-primary btn-module d-flex align-items-center justify-content-center mr-2 button-tambah">
                ...
                {(allowCheck === undefined || allowCheck) && (handleCheckContract !== undefined) && (
                    <button className="btn btn-primary btn-module d-flex align-items-center justify-content-center mr-2 button-check" onClick={(e) => handleCheck()}>
                        <FaSearch className="mr-2" /> <span>Cek Masa Kontrak</span>
                    </button>
                )}
            </div>
        );
    ...
    const renderBulkAction = () => {
        ...
        if (isUpdate && (allowDelete === undefined || allowDelete) && isDelete) {
            return (
                <button type="button" className="btn btn-primary btn-module d-flex align-items-center justify-content-center mr-2 button-hapus" onClick={(e) => handleBulk(e, "hapus")}>
                        ...
                        {(allowDelete === undefined || allowDelete || bulkAction !== undefined) && (
                            <td className="text-center align-middle">
                                <input
                                    ...
                                    checked={checkedItem[item.locationID] !== undefined && checkedItem[item.locationID]}
                                    onChange={handleClick}
                                />
                            </td>
                        )}
                        <td className="text-center align-middle data-item" onClick={(e) => handleView(e, item.locationID)}>
                            {page * limit + index + 1}
                        </td>
                        {columns.map((col, key) => {
                            ...
                            return (
                                <td
                                    ...
                                    onClick={(e) => (col.link === undefined ? handleView(e, item.locationID) : null)}
                                >
                            ...
                            <div className="card-text" style={{ padding: 5, width: '100%' }}>
                                <div className="d-flex" style={{ padding: 10 }} >
                                    ...
                                    <div>
                                        {columns.map((col, key) => {
                                            if (key === 0)
                                                return (<div key={key} className="module-card-title" onClick={(e) => handleView(e, item.locationID)}>{renderValue(col, item[col.key], item)}</div>)
...
export default connect(mapStateToProps)(ListWrapperWifi);
