...
import { deleteDataLeave } from "../../actions/data";
import { GrFormNext } from "react-icons/gr";
import { GrFormPrevious } from "react-icons/gr";
import { MdOutlineDeleteOutline } from "react-icons/md";
import Swal from "sweetalert2";

const ListWrapperLeave = (props) => {
    const navigate = useNavigate();
    const location = useLocation();

    const { img, title, path, url, exportFilename, allowAdd, allowDelete, allowExport, allowFilter, filterSearch, filterDate, allowSearch, bulkAction, handleBulkAction, columns, data, refreshData, exportData, deleteDataLeave, role, roles, condition, allowEdit, handleCustom, handleFilterLeave } = props;
    ...
    const handleCheckedAll = () => {
        ...
        list.forEach((item) => {
            checkedItem[item.leaveID] = checkedAll ? false : true;
    ...
    const handleBulk = (e, type) => {
        e.preventDefault();
        const selectedItems = Object.keys(checkedItem).filter((leaveID) => checkedItem[leaveID]);
        ...
        Swal.fire({
            ...
            if (succeed.value) {
                for (let leaveID of Object.keys(checkedItem)) {
                    if (checkedItem[leaveID]) {
                        if (type === "hapus")
                            await deleteDataLeave({ url, leaveID }).then(() => {
                                refreshData({ url });
                            });
                        else await handleBulkAction(e, type, leaveID);
                    }
                }
                setCheckedAll(false);
                setCheckedItem([]);
    ...
    const handleSearch = (e) => {
        ...
        refreshData({ url, search: e.target.value });
    };
    ...
    const handleLimit = (e, obj) => {
        ...
        setLimit(obj);
        refreshData({ url, search, page, limit: obj });
    };

    const handleNext = (e, page) => {
        e.preventDefault();
        refreshData({ url, search, page: page + 1 });
    };
    ...
    const handleView = (e, leaveID) => {
        e.preventDefault();
        navigate(`${path}/${leaveID}/edit?return_url=${encodeURIComponent(location.pathname)}`);
    };
    ...
    const renderLeftHeader = () => {
        return (
            <div className="d-flex left-flex">
                ...
                {renderBulkAction()}
                {(allowFilter === undefined || allowFilter) && (filterDate || handleFilterLeave !== undefined) && (
                        ...
                        {(allowDelete === undefined || allowDelete || bulkAction !== undefined) && (
                            <td className="text-center align-middle">
                                <input
                                    type="checkbox"
                                    name={item.leaveID}
                                    checked={checkedItem[item.leaveID] !== undefined && checkedItem[item.leaveID]}
                                    onChange={handleClick}
                                />
                            </td>
                        )}
                        <td className="text-center align-middle data-item" onClick={(e) => handleView(e, item.leaveID)}>
                            {page * limit + index + 1}
                        </td>
                        {columns.map((col, key) => {
                            ...
                            return (
                                <td
                                    ...
                                    onClick={(e) => (col.link === undefined ? handleView(e, item.leaveID) : null)}
                                >
                            ...
                            <div className="card-text" style={{ padding: 5, width: '100%' }}>
                                <div className="d-flex" style={{ padding: 10 }} >
                                    ...
                                    <div>
                                        {columns.map((col, key) => {
                                            if (key === 0)
                                                return (<div key={key} className="module-card-title" onClick={(e) => handleView(e, item.leaveID)}>{renderValue(col, item[col.key], item)}</div>)
          ...
          <div class="row">
            ...
            <div class="form-group col-sm-10">
              ...
              {showRange && (
                ...
                {handleFilterLeave !== undefined && handleFilterLeave()}
                <div class="row">
                    <div class="col-2">&nbsp;</div>
...
export default connect(mapStateToProps)(ListWrapperLeave);
