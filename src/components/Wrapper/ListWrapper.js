...
import { deleteDataDailyTask } from "../../actions/data";
...
const ListWrapper = (props) => {
  ...
  const { img, title, path, url, exportFilename, allowAdd, allowDelete, allowExport, allowFilter, filterSearch, filterDate, allowSearch, bulkAction, handleBulkAction, columns, data, refreshData, exportData, deleteDataDailyTask, role, roles, condition, allowEdit, handleCustom, handleAdditionalFilter } = props;
  ...
  const handleCheckedAll = () => {
    ...
    list.forEach((item) => {
      checkedItem[item.dailyTaskID] = checkedAll ? false : true;
      checkedItem[item.dailyTaskID] = checkedAll ? false : true;
    });
  };
  ...
  const handleBulk = (e, type) => {
    e.preventDefault();
    const selectedItems = Object.keys(checkedItem).filter((dailyTaskID) => checkedItem[dailyTaskID]);
    ...
    Swal.fire({
      ...
      if (succeed.value) {
        for (let dailyTaskID of Object.keys(checkedItem)) {
          if (checkedItem[dailyTaskID]) {
            if (type === "hapus")
              await deleteDataDailyTask({ url, dailyTaskID }).then(() => {
                refreshData({ url, search, page, limit, sort, filterSearch, rangeDate });
              });
            else await handleBulkAction(e, type, dailyTaskID);
          }
        }
        setCheckedAll(false);
        setCheckedItem([]);
  ...
  const handleView = (e, dailyTaskID) => {
    e.preventDefault();
    navigate(`${path}/${dailyTaskID}/edit?return_url=${encodeURIComponent(location.pathname)}`);
  };
  ...
  //           <span className="mb-1">Hapus</span>
  //         </button>
  //       }
          ...
          <tr key={index}>
            {(allowDelete === undefined || allowDelete || bulkAction !== undefined) && (
              <td className="text-center align-middle">
                <input
                  type="checkbox"
                  name={item.dailyTaskID}
                  checked={checkedItem[item.dailyTaskID] !== undefined && checkedItem[item.dailyTaskID]}
                  onChange={handleClick}
                />
              </td>
            )}
            <td className="text-center align-middle data-item" onClick={(e) => handleView(e, item.dailyTaskID)}>
              {page * limit + index + 1}
            </td>
            {columns.map((col, key) => {
              ...
              return (
                <td
                  ...
                  onClick={(e) => (col.link === undefined ? handleView(e, item.dailyTaskID) : null)}
                >
              ...
              <div className="card-text" style={{ padding: 5, width: '100%' }}>
                <div className="d-flex" style={{ padding: 10 }} >
                  ...
                  <div>
                    {columns.map((col, key) => {
                      if (key === 0)
                        return (<div key={key} className="module-card-title" onClick={(e) => handleView(e, item.dailyTaskID)}>{renderValue(col, item[col.key], item)}</div>)
