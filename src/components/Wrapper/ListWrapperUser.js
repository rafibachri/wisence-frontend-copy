...
const ListWrapperUser = (props) => {
    ...
    const handleBulk = (e, type) => {
        e.preventDefault();
        const selectedItems = Object.keys(checkedItem).filter((userID) => checkedItem[userID]);
        ...
        Swal.fire({
            ...
            if (succeed.value) {
                for (let userID of Object.keys(checkedItem)) {
                    if (checkedItem[userID]) {
                        if (type === "hapus")
                            await deleteDataDailyUser({ url, userID }).then(() => {
                                refreshData({ url, search, page, limit, sort, filterSearch, rangeDate });
                            });
                        else await handleBulkAction(e, type, userID);
                    }
                }
                setCheckedAll(false);
                setCheckedItem([]);
    ...
    const handleView = (e, userID) => {
        e.preventDefault();
        navigate(`${path}/${userID}/edit?return_url=${encodeURIComponent(location.pathname)}`);
    };
                        ...
                        {(allowDelete === undefined || allowDelete || bulkAction !== undefined) && (
                            <td className="text-center align-middle">
                                <input
                                    type="checkbox"
                                    name={item.userID}
                                    checked={checkedItem[item.userID] !== undefined && checkedItem[item.userID]}
                                    onChange={handleClick}
                                />
                            </td>
                        )}
                        <td className="text-center align-middle data-item" onClick={(e) => handleView(e, item.userID)}>
                            {page * limit + index + 1}
                        </td>
                        {columns.map((col, key) => {
                            ...
                            return (
                                <td
                                    ...
                                    onClick={(e) => (col.link === undefined ? handleView(e, item.userID) : null)}
                                >
                            ...
                            <div className="card-text" style={{ padding: 5, width: '100%' }}>
                                <div className="d-flex" style={{ padding: 10 }} >
                                    ...
                                    <div>
                                        {columns.map((col, key) => {
                                            if (key === 0)
                                                return (<div key={key} className="module-card-title" onClick={(e) => handleView(e, item.userID)}>{renderValue(col, item[col.key], item)}</div>)
...
export default connect(mapStateToProps)(ListWrapperUser);
