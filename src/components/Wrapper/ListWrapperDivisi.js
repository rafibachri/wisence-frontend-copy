...
import { deleteDataDivisi } from "../../actions/data";
import { GrFormNext } from "react-icons/gr";
import { GrFormPrevious } from "react-icons/gr";
import { MdOutlineDeleteOutline } from "react-icons/md";
import Swal from "sweetalert2";

const ListWrapperDivisi = (props) => {
    const navigate = useNavigate();
    const location = useLocation();

    const { img, title, path, url, exportFilename, allowAdd, allowDelete, allowExport, allowFilter, filterSearch, filterDate, allowSearch, bulkAction, handleBulkAction, columns, data, refreshData, exportData, deleteDataDivisi, role, roles, condition, allowEdit, handleCustom, handleAdditionalFilter } = props;
    ...
    const handleCheckedAll = () => {
        ...
        list.forEach((item) => {
            checkedItem[item.divisionID] = checkedAll ? false : true;
    ...
    const handleBulk = (e, type) => {
        e.preventDefault();
        const selectedItems = Object.keys(checkedItem).filter((divisionID) => checkedItem[divisionID]);
        ...
        Swal.fire({
            ...
            if (succeed.value) {
                for (let divisionID of Object.keys(checkedItem)) {
                    if (checkedItem[divisionID]) {
                        if (type === "hapus")
                            await deleteDataDivisi({ url, divisionID }).then(() => {
                                refreshData({ url, search, page, limit, sort, filterSearch, rangeDate });
                            });
                        else await handleBulkAction(e, type, divisionID);
                    }
                }
                setCheckedAll(false);
                setCheckedItem([]);
    ...
    const handleView = (e, divisionID) => {
        e.preventDefault();
        navigate(`${path}/${divisionID}/edit?return_url=${encodeURIComponent(location.pathname)}`);
    };
                        ...
                        {(allowDelete === undefined || allowDelete || bulkAction !== undefined) && (
                            <td className="text-center align-middle">
                                <input
                                    ...
                                    checked={checkedItem[item.divisionID] !== undefined && checkedItem[item.divisionID]}
                                    onChange={handleClick}
                                />
                            </td>
                        )}
                        <td className="text-center align-middle data-item" onClick={(e) => handleView(e, item.divisionID)}>
                            {page * limit + index + 1}
                        </td>
                        {columns.map((col, key) => {
                            ...
                            return (
                                <td
                                    ...
                                    onClick={(e) => (col.link === undefined ? handleView(e, item.divisionID) : null)}
                                >
                            ...
                            <div className="card-text" style={{ padding: 5, width: '100%' }}>
                                <div className="d-flex" style={{ padding: 10 }} >
                                    ...
                                    <div>
                                        {columns.map((col, key) => {
                                            if (key === 0)
                                                return (<div key={key} className="module-card-title" onClick={(e) => handleView(e, item.divisionID)}>{renderValue(col, item[col.key], item)}</div>)
...
export default connect(mapStateToProps)(ListWrapperDivisi);
