...
import { deleteDataAttendance } from "../../actions/data";
import { GrFormNext } from "react-icons/gr";
import { GrFormPrevious } from "react-icons/gr";
import { MdOutlineDeleteOutline } from "react-icons/md";
import Swal from "sweetalert2";

const ListWrapperAttendance = (props) => {
    const navigate = useNavigate();
    const location = useLocation();

    const { img, title, path, url, exportFilename, allowAdd, allowDelete, allowExport, allowFilter, filterSearch, filterDate, allowSearch, bulkAction, handleBulkAction, columns, data, refreshData, exportData, deleteDataAttendance, role, roles, condition, allowEdit, handleCustom, handleAdditionalFilter } = props;
    ...
    useEffect(() => {
        if (rangeDate !== undefined && rangeDate !== null) {
            setRangeString(moment(rangeDate[0].startDate).format("YYYY-MM-DD") + "/" + moment(rangeDate[0].endDate).format("YYYY-MM-DD"));
        }
    }, [rangeDate]);
    ...
    const handleSort = (e) => {
        ...
        setSortBy(newSort);
        setSort(obj);
        refreshData({ url, search, page, limit, sort: obj, filterSearch });
    };

    const handleCheckedAll = () => {
        setCheckedAll(checkedAll ? false : true);
        list.forEach((item) => {
            checkedItem[item.attendanceID] = checkedAll ? false : true;
    ...
    const handleBulk = (e, type) => {
        e.preventDefault();
        const selectedItems = Object.keys(checkedItem).filter((attendanceID) => checkedItem[attendanceID]);
        ...
        Swal.fire({
            ...
            if (succeed.value) {
                for (let attendanceID of Object.keys(checkedItem)) {
                    if (checkedItem[attendanceID]) {
                        if (type === "hapus")
                            await deleteDataAttendance({ url, attendanceID }).then(() => {
                                refreshData({ url, search, page, limit, sort, filterSearch, rangeDate });
                            });
                        else await handleBulkAction(e, type, attendanceID);
                    }
                }
                setCheckedAll(false);
                setCheckedItem([]);
    ...
    const handleLimit = (e, obj) => {
        ...
        setLimit(obj);
        refreshData({ url, search, page, limit: obj, sort, filterSearch });
    };

    const handleNext = (e, page) => {
        e.preventDefault();
        refreshData({ url, search, page: page + 1, limit, sort, filterSearch });
    };
    ...
    const handleView = (e, attendanceID) => {
        e.preventDefault();
        navigate(`${path}/${attendanceID}/edit?return_url=${encodeURIComponent(location.pathname)}`);
    };
    ...
    //                     <span className="mb-1">Delete</span>
    //                 </button>
    //             }
    //             <div className="dropdown-menu">
    //                 {(allowDelete === undefined || allowDelete) && isDelete && (
    //                     <div className="btn dropdown-item border-bottom text-left" onClick={(e) => handleBulk(e, "hapus")}>
    //                         Delete Data
    //                     </div>
    //                 )}
                    ...
                    <tr key={index}>
                        {(allowDelete === undefined || allowDelete || bulkAction !== undefined) && (
                            <td className="text-center align-middle">
                                <input
                                    type="checkbox"
                                    name={item.attendanceID}
                                    checked={checkedItem[item.attendanceID] !== undefined && checkedItem[item.attendanceID]}
                                    onChange={handleClick}
                                />
                            </td>
                        )}
                        <td className="text-center align-middle data-item" onClick={(e) => handleView(e, item.attendanceID)}>
                            {page * limit + index + 1}
                        </td>
                        {columns.map((col, key) => {
                            ...
                            return (
                                <td
                                    ...
                                    onClick={(e) => (col.link === undefined ? handleView(e, item.attendanceID) : null)}
                                >
                            ...
                            <div className="card-text" style={{ padding: 5, width: '100%' }}>
                                <div className="d-flex" style={{ padding: 10 }} >
                                    ...
                                    <div>
                                        {columns.map((col, key) => {
                                            if (key === 0)
                                                return (<div key={key} className="module-card-title" onClick={(e) => handleView(e, item.attendanceID)}>{renderValue(col, item[col.key], item)}</div>)
    ...
    const renderFilter = () => {
        ...
        return (
            <div className="filter mb-1">
                ...
                {filterDate && (
                    <div class="row">
                        <div class="col-sm-2" style={{ paddingTop: 4, marginBottom: 10 }}>Tanggal Kehadiran</div>
                        <div class="form-group col-sm-10">
                            ...
                            {showRange && (
                                <div className="date-container">
                                    <DateRange onChange={(item) => handleSelect(item)} showSelectionPreview={true} moveRangeOnFirstSelection={false} months={1} ranges={rangeDate} inputRanges={[]} direction="horizontal" preventSnapRefocus={true} calendarFocus="backwards" />
                                </div>
                            )}
                        </div>
                    </div>
                ...
                <div class="row">
                    ...
                    <div class="form-group col-10">
                        <button className="btn btn-search d-flex align-items-center justify-content-center mr-2" onClick={e => handleRefresh(e)}>
                            <FaSearch className="mr-2" /> <span> Cari</span>
                        </button>
                    </div>
                </div>
            </div >
...
ListWrapperAttendance.propTypes = {
...
export default connect(mapStateToProps)(ListWrapperAttendance);
