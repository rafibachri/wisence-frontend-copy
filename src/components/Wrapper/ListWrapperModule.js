...
import { deleteDataModule } from "../../actions/data";

const ListWrapperModule = (props) => {
    const navigate = useNavigate();
    const location = useLocation();

    const { img, title, path, url, exportFilename, allowAdd, allowDelete, allowExport, allowFilter, filterSearch, filterDate, allowSearch, bulkAction, handleBulkAction, columns, data, refreshData, exportData, deleteDataModule, role, roles, condition, allowEdit, handleCustom, handleAdditionalFilter } = props;
    ...
    const handleCheckedAll = () => {
        ...
        list.forEach((item) => {
            checkedItem[item.moduleID] = checkedAll ? false : true;
    ...
    const handleBulk = (e, type) => {
        e.preventDefault();
        const selectedItems = Object.keys(checkedItem).filter((moduleID) => checkedItem[moduleID]);
        ...
        swal({
            ...
            if (succeed) {
                for (let moduleID of Object.keys(checkedItem)) {
                    if (checkedItem[moduleID]) {
                        if (type === "hapus")
                            await deleteDataModule({ url, moduleID }).then(() => {
                                refreshData({ url, search, page, limit, sort, filterSearch, rangeDate });
                            });
                        else await handleBulkAction(e, type, moduleID);
                    }
                }
                setCheckedAll(false);
                setCheckedItem([]);
    ...
    const handleView = (e, moduleID) => {
        e.preventDefault();
        navigate(`${path}/${moduleID}/edit?return_url=${encodeURIComponent(location.pathname)}`);
    };
                        ...
                        {(allowDelete === undefined || allowDelete || bulkAction !== undefined) && (
                            <td className="text-center align-middle">
                                <input
                                    type="checkbox"
                                    name={item.moduleID}
                                    checked={checkedItem[item.moduleID] !== undefined && checkedItem[item.moduleID]}
                                    onChange={handleClick}
                                />
                            </td>
                        )}
                        <td className="text-center align-middle data-item" onClick={(e) => handleView(e, item.moduleID)}>
                            {page * limit + index + 1}
                        </td>
                        {columns.map((col, key) => {
                            ...
                            return (
                                <td
                                    ...
                                    onClick={(e) => (col.link === undefined ? handleView(e, item.moduleID) : null)}
                                >
                            ...
                            <div className="card-text" style={{ padding: 5, width: '100%' }}>
                                <div className="d-flex" style={{ padding: 10 }} >
                                    ...
                                    <div>
                                        {columns.map((col, key) => {
                                            if (key === 0)
                                                return (<div key={key} className="module-card-title" onClick={(e) => handleView(e, item.moduleID)}>{renderValue(col, item[col.key], item)}</div>)
...
export default connect(mapStateToProps)(ListWrapperModule);
