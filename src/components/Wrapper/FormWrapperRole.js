...
const FormWrapperRole = (props) => {
    // const pdf = useRef(null);
    const location = useLocation();

    const [searchParams] = useSearchParams();
    const { id, roleID, img, title, path, type, children, handleSave, role, roles, allowAdd, allowUpdate, allowBack } = props;
        ...
        return (
            <div className="module d-flex justify-content-between" style={{ background: 'linear-gradient(to right, #1E90FF, #87CEEB)' }}>
                ...
                <div className="d-flex">
                    ...
                    {roleID !== undefined && (allowUpdate === undefined || allowUpdate) &&
                        isUpdate &&
                        (saving ? (
                            <div className="text-center">Menyimpan...</div>
                        ) : (
                    ...
                    {allowBack && returnUrl !== undefined && returnUrl !== null && (
                        <Link to={`${returnUrl}`} className="btn btn-back ml-2 d-flex align-items-center justify-content-center">
                            ...
                            {roleID === undefined ? "Kembali" : "Kembali"}
                        </Link>
                    )}
                </div>
            </div>
...
export default connect(mapStateToProps)(FormWrapperRole);
